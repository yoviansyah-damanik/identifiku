...
class TeacherRegistration extends Component
{
    ...
    public function render()
    {
        ...
        return view('pages.auth.registration-step.teacher-registration', compact('schools'))
            ->title(__('Register'));
    }
    ...
    public function submit()
    {
        ...
        try {
            ...
            if ($school) {
                if (!$school->is_open) {
                    $this->alert('warning', __('The selected school cannot add teachers. Please contact the School Administator for more details.'));
                    $this->isLoading = false;
                    return;
                }

                return $this->redirectRoute('registration.teacher.final', ['school' => $this->school, 'token' => $this->token], navigate: true);
            }
