    ...
    <div class="flex flex-col items-start w-full gap-3 lg:gap-4 lg:flex-row">
        <div class="flex flex-col w-full gap-3 lg:w-[24rem] xl:w-[36rem] sm:flex-row lg:flex-col lg:gap-4">
            ...
            <div class="w-full p-6 overflow-hidden bg-white rounded-lg shadow-md sm:flex-1 lg:p-8">
                ...
                <div class="max-h-[40dvh] overflow-auto">
                    @forelse ($assessment->class->assessments()
                    ->where('student_id','!=', $assessment->student_id)->where('quiz_id',$assessment->quiz->id)->get() as $item)
        ...
        <div class="flex flex-col flex-1 w-full gap-3 lg:gap-4">
            <div class="p-6 overflow-hidden bg-white rounded-lg shadow-md lg:p-8">
                <div class="text-lg font-bold text-center text-primary-500">
                    {{ __('Assessment Result') }}
                </div>
                @if ($assessment->quiz->type == 'multipleIntelligenceType')
                    <div class="mb-5 font-light text-center">
                        {{ __('Here are your multiple intelligences in order of percentage based on the assessment results') }}
                    </div>
                @endif
                        ...
                        @foreach ($assessment->result->details as $detail)
                            <div>
                                <div class="flex items-center justify-between gap-3 mb-1 lg:gap-4">
                                    ...
                                    <div class="text-sm">
                                        {{ GeneralHelper::numberFormat(($detail->score / $detail->max_score) * 100) }}%
                                    </div>
                                </div>
                                <div @class([
                                    'relative w-full h-4 overflow-hidden rounded-full shadow bg-red-50',
                                    '!bg-primary-50' =>
                                        $loop->iteration <= 3 && $assessment->result->details->count() > 3,
                                ])>
                                    <div style="width: {{ ($detail->score / $detail->max_score) * 100 > 0 ? ($detail->score / $detail->max_score) * 100 . '%' : '0px' }}"
                                        @class([
