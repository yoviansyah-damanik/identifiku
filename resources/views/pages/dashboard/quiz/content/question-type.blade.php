<li x-data="{ id: '{{ $type->id }}', isActive: false, groupActive: '' }"
    ...
    x-on:click="typeActive = id" wire:click="$dispatch('setTypeActive',{ type: '{{ $type->id }}'})"
    x-init="$watch('typeActive', val => isActive = val == id ? true : false)" {{-- :class="isActive ? 'bg-primary-50' : 'bg-white'" --}} x-sort:item="'{{ $type->id }}'">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center gap-1 bg-gradient-to-r from-transparent from-10% to-primary-100 px-7"
            ...
            <x-tooltip :title="__('Edit :edit', ['edit' => __(':type Type', ['type' => __('Question')])])">
                <x-button color="yellow" icon="i-ph-pen" size="sm"
                    wire:click="$dispatch('setEditType', { type: '{{ $type->id }}'})"
            ...
            <x-tooltip :title="__('Delete :delete', ['delete' => __(':type Type', ['type' => __('Question')])])">
                <x-button color="red" icon="i-ph-trash" size="sm"
                    wire:click="$dispatch('setDeleteType', { type: '{{ $type->id }}'})"
                    x-on:click="$dispatch('toggle-delete-type-modal')" />
            </x-tooltip>
