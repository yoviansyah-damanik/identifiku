<form autocomplete="off" wire:submit.prevent='submit' class="w-full space-y-3 sm:space-y-4">
    ...
    <div class="flex items-center gap-3 sm:gap-4">
        ...
        <x-form.select-with-search :loading="$isLoading" block class="flex-1" block searchVar="regencySearch"
            :items="$regencies" wire:model="regencySearch" error="{{ $errors->first('regency') }}" :label="__('Choose a :item', ['item' => __('Regency')])" />
    </div>
    <div class="flex items-center gap-3 sm:gap-4">
        ...
        <x-form.select-with-search :loading="$isLoading" block class="flex-1" block searchVar="villageSearch"
            :items="$villages" wire:model="villageSearch" error="{{ $errors->first('village') }}" :label="__('Choose a :item', ['item' => __('Village')])" />
    </div>
    <div class="flex items-center gap-3 sm:gap-4">
